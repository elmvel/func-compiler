...
private:
  ...
  void perform_instr(GmInstr &&next) {
    ...
    case POP: {
      // DWG: Had to implement this for my compiler.
      int num = std::get<int>(next.data);
      for (int i = 0; i < num; ++i) {
        curr_stack.pop();
      }
    } break;
    ...
    case MKAPSPJ: {
      // DWG: For some reason, this implementation ordered this operation differently.
      auto f = std::move(curr_stack.top());
      curr_stack.pop();
      auto x = std::move(curr_stack.top());
      curr_stack.pop();
...
public: // DWG: This is for inspection purposes
  int hash = -1;
  int arity = -1;
